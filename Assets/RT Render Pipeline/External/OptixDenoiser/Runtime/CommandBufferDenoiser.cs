...
namespace UnityEngine.Rendering.Denoising
{
    ...
    public class CommandBufferDenoiser : DenoiserBase
    {
        ...
        unsafe void* ptrColor;
        StringBuilder strBuilder = new StringBuilder("color", 7);
        ...
        public State DenoiseRequest(CommandBuffer cmd, string type, RenderTexture src)
        {
            ...
            DoReadBack(cmd, src, ColorReadBack);
        ...
        //    Texture2D m_DenoisedHistory = new Texture2D(m_RequestWidth, m_RequestHeight, format, TextureCreationFlags.None);
        ...
        //    return State.Success;
        //}
        unsafe public State GetResults(CommandBuffer cmd, ref Texture2D dst)
        {
            //cmd.CopyTexture(m_BeautyData, 0, 0, 0, 0, m_RequestWidth, m_RequestHeight, dst, 0, 0, 0, 0);
            // dst.SetPixelData<Vector4>(m_BeautyData, 0);
            dst.LoadRawTextureData(m_BeautyData);
            dst.Apply();
            //InternalReset();
            return State.Success;
        }
        ...
        unsafe internal State ExecuteDenoiseRequest()
        {
            ...
            if (!isPointed) { 
                ptrColor = NativeArrayUnsafeUtility.GetUnsafePtr(m_BeautyData);
                isPointed = true;
            }
            // var format = GraphicsFormat.R32G32B32A32_SFloat;

            // 开辟内存
            int status  = udnDenoiseBuffer(m_DenoiserBackend, strBuilder, ptrColor, GraphicsFormat.R32G32B32A32_SFloat);
        ...
        internal void DoReadBack(CommandBuffer cmd, RenderTexture src, Action<AsyncGPUReadbackRequest> callback)
        {
        ...
        //NativeArray<Vector4> m_BeautyData = new NativeArray<Vector4>(Screen.height * Screen.width, Allocator.Persistent, NativeArrayOptions.UninitializedMemory);
        NativeArray<Vector4> m_BeautyData = new NativeArray<Vector4>(2088 * 1044, Allocator.Persistent, NativeArrayOptions.UninitializedMemory);
